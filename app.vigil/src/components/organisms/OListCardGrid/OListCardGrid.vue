...
<template>
  ...
  >
    <div
      v-for="item in items"
      class="border-line bg-canvas flex flex-col gap-3 rounded-xl border p-4"
    ...
    >
      ...
      <div>
        <p
          v-if="amount"
          :class="['text-heading text-xl font-bold', amount.align === 'right' ? 'text-right' : 'text-left']"
        >
          {{ amountValue(item) }}
        </p>

        <p class="text-muted text-sm">{{ fieldValue(item, title.valueKey) }}</p>
      </div>
